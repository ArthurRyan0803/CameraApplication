...
PDRCamerasFactory::PDRCamerasFactory() : AbstractCamerasFactory(), cameras_infos_({}), cameras_nums_(MAX_CAMERA_CONNECTIONS)
{
...
std::vector<std::string> PDRCamerasFactory::enumerateCamerasIDs()
{
...
std::shared_ptr<Camera> PDRCamerasFactory::createCamera(const std::string& sn) const
{
	if (cameras_infos_.find(sn) == cameras_infos_.end())
		throw VSensorException("Invalid sn!", -1, __FILE__, __LINE__);
